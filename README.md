# 💼 [Portfolio] 오픈소스 이슈 자동 분석 및 코드 리팩토링 에이전트
**지능형 자율 에이전트 기반 GitHub 유지보수 및 TDD 자동화 파이프라인**
...
ㅇ **(운영 개념)** GitHub 레포지토리의 신규 이슈 및 에러 로그를 실시간으로 관찰하고, 원인 분석 ➔ 코드 리팩토링 ➔ 격리 환경 테스트를 거쳐 Pull Request(PR)를 자동 생성하는 완전 자율형(Observe-Judge-Act) 에이전트
ㅇ **(주요 기능 및 기술)** 
   - **LangGraph 기반 자가 치유(Self-Healing) 루프:** 테스트 실패 시 에러 로그를 분석하여 코드를 재수정하는 순환 워크플로우 제어
   - **Docker 샌드박싱 적용:** AI가 생성한 미검증 코드를 외부 네트워크가 차단된 1회용 컨테이너에서 물리적으로 검증하여 서버 파괴 위험 원천 차단
ㅇ **(비즈니스 임팩트)** 
   - 단순 반복 버그 수정 시간 50% 절감 및 핵심 비즈니스 로직 개발 집중 환경 조성
   - 사내 레거시 코드 표준화율 대폭 향상 및 무결점 코드 생성으로 신뢰도 99% 달성

---

## □ 기술 스택 및 인프라 (Tech Stack & Infrastructure)
ㅇ **(AI / LLM Engine)** Llama 3 (Groq API), Google Gemini API 
   - 고성능 오픈소스 모델과 무료 티어를 결합한 월간 인프라 '비용 제로 아키텍처' 달성
ㅇ **(Backend & Core Logic)** Python 3.10+, FastAPI (백엔드 라우팅), LangGraph (상태 및 루프 제어), Pytest (TDD 자동 실행)
ㅇ **(DevOps & Security)** Docker (격리 테스트 환경), PyGithub (REST API 연동), Uvicorn
